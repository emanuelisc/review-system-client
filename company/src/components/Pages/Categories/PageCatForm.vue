      ...
      <md-card-content>
        <div class="md-layout">
          ...
          <div class="md-layout-item md-size-50 text-right">
            <md-button
              v-if="this.$route.name != 'page-cat-new'"
              @click="onUpdate()"
...
export default {
  ...
  methods: {
    ...
    onDelete() {
      ...
      if (r == true) {
        this.$store
          .dispatch("deleteCategory", this.$route.params.id)
          .then(() => {
            this.notifyVue("Kategorija sėkmingai pašalinta!", "success");
            this.$router.push({ name: "page-cat-list" });
          })
          .catch(err =>
    ...
    onUpdate() {
      ...
      axios
        .patch(`page/categories/${this.$route.params.id}/`, object)
        .then(response => {
    ...
    getData() {
      axios
        .get(`page/categories/${this.$route.params.id}/`)
        .then(response => {
  ...
  created() {
    if (this.$route.name != "page-cat-new") {
      this.getData();
