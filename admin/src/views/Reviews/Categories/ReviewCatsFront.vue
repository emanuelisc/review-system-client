<template>
  <div class="content container mb-4">
    <div class="md-layout">
        ...
        <p class="page-desc">
            Pasirinkite norimą atsiliepimų kategoriją
        </p>
      <div class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
        <md-list>
          <md-list-item v-for="item in searched" v-bind:key="item.id">
            <a v-bind:href="'/atsiliepimai/?category=' + item.id">{{ item.name }}</a>
          </md-list-item>
        </md-list>
      </div>
    </div>
...
export default {
  components: {},
  computed: mapGetters(["allReviewCategories"]),
  methods: {
    ...mapActions(["fetchReviewCats"]),
    getData() {
      axios
        .get("review/categories/")
        .then(response => {
          this.searched = response.data.results;
          this.fetchReviewCats(response.data.results);
        })
        .catch(err => {
  ...
  created() {
    ...
    this.searched = this.allReviewCategories;
