...
export default {
  ...
  computed: mapGetters(["allReviewTags"]),
  methods: {
    ...mapActions(["fetchReviewTags"]),
    ...
    searchName() {
      this.searched = searchByName(this.allReviewTags, this.search);
    },
    getData() {
      axios
        .get("review/tags/")
        .then(response => {
          ...
          this.fetchReviewTags(response.data.results);
        })
        .catch(err => {
          this.notifyVue("Nepavyko gauti sąrašo.", "danger");
          console.log(err);
        });
    }
  },
  created() {
    if (this.allReviewTags.length == 0) {
      this.getData();
      console.log("Zero");
    } else {
      console.log("Ok");
    }
    this.searched = this.allReviewTags;
