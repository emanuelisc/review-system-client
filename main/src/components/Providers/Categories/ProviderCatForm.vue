      ...
      <md-card-content>
        <div class="md-layout">
          ...
          <div class="md-layout-item md-size-50 text-right">
            <md-button
              v-if="this.$route.name != 'provider-cat-new'"
              @click="onUpdate()"
...
export default {
  ...
  methods: {
    ...
    onDelete() {
      ...
      if (r == true) {
        this.$store
          .dispatch("deleteProviderCategory", this.$route.params.id)
          .then(() => {
            this.notifyVue("Kategorija sėkmingai pašalinta!", "success");
            this.$router.push({ name: "provider-cat-list" });
          })
          .catch(err =>
    ...
    onUpdate() {
      ...
      axios
        .patch(`provider/categories/${this.$route.params.id}/`, object)
        .then(response => {
    ...
    onCreate() {
      ...
      object["name"] = this.category.name;
      axios
        .post("provider/categories/", object)
        .then(response => {
    ...
    getData() {
      axios
        .get(`provider/categories/${this.$route.params.id}/`)
        .then(response => {
  ...
  computed: mapGetters(["oneCategory", "allCategories"]),
  created() {
    if (this.$route.name != "provider-cat-new") {
      this.getData();
